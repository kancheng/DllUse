@echo off
REM 編譯 C++ 調用 C# DLL 的程序（支持 32 位和 64 位）
REM 需要啟用 CLR 支持（/clr）

echo === 編譯 C++ 調用 C# DLL 程序 (32位和64位) ===
echo.
...
REM 清理舊文件 (64位)
if exist "x64\main.exe" del "x64\main.exe"
if exist "x64\main.obj" del "x64\main.obj"

echo 編譯 64 位 main.cpp (使用 /clr)...
cl /clr /EHsc /MD main.cpp /Fe:x64\main.exe /link /SUBSYSTEM:CONSOLE
...
echo.
echo 64 位編譯成功！
echo 可執行文件: %SCRIPT_DIR%x64\main.exe
echo.
...
REM 清理舊文件 (32位)
if exist "x86\main.exe" del "x86\main.exe"
if exist "x86\main.obj" del "x86\main.obj"

echo 編譯 32 位 main.cpp (使用 /clr)...
cl /clr /EHsc /MD main.cpp /Fe:x86\main.exe /link /SUBSYSTEM:CONSOLE
...
echo.
echo 32 位編譯成功！
echo 可執行文件: %SCRIPT_DIR%x86\main.exe
echo.
...
echo ========================================
echo.
echo 64 位程序: x64\main.exe
echo 32 位程序: x86\main.exe
echo.
echo 注意: 運行前請確保 C# DLL 已編譯並位於正確位置
echo.
